...
const CreateOrUpdateMoodTracker = catchAsync(async (req, res) => {
  ...
  const result = await MoodTrackerServices.CreateOrUpdateMoodTrackerToDB(
    userId,
    moodTrackerData,
  );
  sendResponse(res, {
...
const getMyMoodTrackerHistories = catchAsync(async (req, res) => {
  const { id: userId } = req.user;
  const result = await MoodTrackerServices.getMyMoodTrackerHistoriesFromDB(
    userId,
    req.query,
  );
  sendResponse(res, {
...
const deleteMoodTrackerHistoryById = catchAsync(async (req, res) => {
  const { id } = req.params;
  const result =
    await MoodTrackerServices.deleteMoodTrackerHistoryByIdFromDB(id);
  sendResponse(res, {
    success: true,
    statusCode: 200,
